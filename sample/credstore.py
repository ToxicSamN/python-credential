...
class Credential:
    __log = logging.getLogger('Credstore.Credential')

    def __init__(self, username):
        self.__log.debug(f"RSAPriv: {os.environ.get('RSAPrivateFile' or None)}, RSASecret: {os.environ.get('RSASecret' or None)}")
        self.username = username
        self.session = requests.Session()
        self.session.verify = False

        # typically you will want to use valid HTTPS certificates
    ...
    def get_credential(self, dev=False):
        if dev:
            credstore_uri = f'https://credstore-dev/credentialstore/GetCredential?ClientId={os.environ["ClientId"]}&username={self.username}'
        else:
            credstore_uri = f'https://credstore/credentialstore/GetCredential?ClientId={os.environ["ClientId"]}&username={self.username}'

        self.__log.debug(f"Credstore URI: {credstore_uri}")

        response = self.session.get(url=credstore_uri)
        data = json.loads(response.text)
        self.__log.debug(f"API results: {data.__str__()}")
        self.decipher(
